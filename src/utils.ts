...
export function convertDeeplinkToUniversalLink(link: string, walletUniverslLink: string): string {
  const search = new URL(link).search;
  const url = new URL(walletUniverslLink);

  if(isTelegramUrl(walletUniverslLink)) {
